<?php
        ...
        if(isset($_GET['motCle'],$_GET['select'])){
            ...
            $req = $conn->prepare("SELECT * FROM ventes WHERE $select like  '%$mc%' ORDER BY id_vente DESC  LIMIT $size OFFSET $offset"); 

        }else{
            $req = $conn->prepare("SELECT * FROM ventes ORDER BY id_vente DESC  LIMIT $size OFFSET $offset "); 
        }
        ...
        if(isset($_GET['motCle'],$_GET['select']))
        $req2 = $conn->prepare("SELECT COUNT(*) AS NMBRE_SUP FROM ventes WHERE $select like  '%$mc%' ORDER BY id_vente DESC  ");
        else
        $req2 = $conn->prepare("SELECT COUNT(*) AS NMBRE_SUP FROM ventes ORDER BY id_vente DESC ");
        $req2->execute();
